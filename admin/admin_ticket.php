...
$ticket['anonymized'] = ($ticket['name'] == $hesklang['anon_name'] &&
    ...
    $ticket['ip'] == $hesklang['anon_IP']) ? true : false;
                                ...
                                <form class="form" method="post" action="admin_ticket.php">
                                    <section class="param calendar">
                                        <div class="calendar--button">
                                            ...
                                            <input name="new-due-date" data-datepicker-position="bottom left" value="<?php echo $datepicker_due_date; ?>" type="text" class="datepicker">
                                        </div>
                                        ...
                                        <div class="options" style="text-align: left">
                                            <div>
                                                <div class="radio-custom">
                                                    <input type="radio" name="mode" id="modeadd" value="1" checked>
                                                    <label for="modeadd">
            ...
            function hesk_printCanned()
            {
                ...
                ?>
                    <script language="javascript" type="text/javascript">
                        ...
                        myMsg
                        =
                        myMsg.replace(/%%HESK_ID%%/g,
                        '<?php echo hesk_jsString($ticket['id']); ?>');
                        myMsg
                        =
                        myMsg.replace(/%%HESK_TRACKID%%/g,
                        '<?php echo hesk_jsString($ticket['trackid']); ?>');
                        myMsg
                        =
                        myMsg.replace(/%%HESK_TRACK_ID%%/g,
                        '<?php echo hesk_jsString($ticket['trackid']); ?>');
                        myMsg
                        =
                        myMsg.replace(/%%HESK_NAME%%/g,
                        '<?php echo hesk_jsString($ticket['name']); ?>');
                        myMsg
                        =
                        myMsg.replace(/%%HESK_FIRST_NAME%%/g,
                        '<?php echo hesk_jsString(hesk_full_name_to_first_name($ticket['name'])); ?>');
                        myMsg
                        =
                        myMsg.replace(/%%HESK_EMAIL%%/g,
                        '<?php echo hesk_jsString($ticket['email']); ?>');
                        myMsg
                        =
                        myMsg.replace(/%%HESK_OWNER%%/g,
                        '<?php echo hesk_jsString(isset($admins[$ticket['owner']]) ? $admins[$ticket['owner']] : ''); ?>');
                        <?php
                        for ($i = 1; $i <= 50; $i++) {
                        ...
                        {
                        if
                        (document.getElementById('moderep').checked)
                        {
                        <?php if ($hesk_settings['staff_ticket_formatting'] == 2) : ?>
                        tinymce.get("message").setContent('');
                        tinymce.get("message").execCommand('mceInsertRawHTML',
                        false,
                        myMsg);
                        <?php else : ?>
                        ...
                        =
                        '
                    <textarea name="message" id="message" placeholder="<?php echo $hesklang['type_your_message']; ?>">' + myMsg + '</textarea>';
                <?php endif; ?>
                } else {
                <?php if ($hesk_settings['staff_ticket_formatting'] == 2) : ?>
